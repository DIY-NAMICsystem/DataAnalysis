# Author: SC
# Based code written by JHL and KN

import pandas as pd

## 1a. Counting during Specific Windows (ex: delay window) with 4 digit event codes

def counts_during_window4(m_parsed_dt_df, start_parsetime, window_of_interest, daily=True):
    """
    ...
    for i in range(len(box_arr)):  # for all the boxes, (outermost index is box number)
        ...
        if daily:
            metric_code = m_date + " counter: x" + window_of_interest[0][-4:]  # last 4 digits of counter_code
        else:
            metric_code = m_hour + " counter: x" + window_of_interest[0][-4:]  # last 4 digits of counter_code
...
## 1b. Counting during Specific Windows (ex: delay window) with 3 digit event codes

def counts_during_window3(m_parsed_dt_df, start_parsetime, window_of_interest, daily=True):
    """
    ...
    for i in range(len(box_arr)):  # for all the boxes, (outermost index is box number)
        ...
        # Metric Code: will become the name of index (for dataframe)
        if daily:
            metric_code = m_date + " counter: xx" + window_of_interest[0][-3:]  # last 3 digits of counter_code
        else:
            metric_code = m_hour + " counter: xx" + window_of_interest[0][-3:]  # last 3 digits of counter_code
...
## 2a. Counting Events! with 4 digit event codes

def count_events4(m_parsed_dt_df, start_parsetime, code_of_interest, daily=True):
    """
    ...
    for i in range(len(box_arr)):  # for all the boxes, (outermost index is box number)
        ...
        # Metric Code: will become the name of index (for dataframe)
        if daily:
            metric_code = m_date + " event_code: x" + code_of_interest[0][-4:]  # last 4 digits of event_code

        else:
            metric_code = m_hour + " event_code: x" + code_of_interest[0][-4:]  # last 4 digits of event_code
...
## 2b. Counting Events! with 3 digit event codes

def count_events3(m_parsed_dt_df, start_parsetime, code_of_interest, daily=True):
    """
    ...
    for i in range(len(box_arr)):  # for all the boxes, (outermost index is box number)
        ...
        # Metric Code: will become the name of index (for dataframe)
        if daily:
            metric_code = m_date + " event_code: x" + code_of_interest[0][-3:]  # last 3 digits of event_code

        else:
            metric_code = m_hour + " event_code: x" + code_of_interest[0][-3:]  # last 3 digits of event_code
...
## 2c. Counting Events! with 5 digit event codes

def count_events5(m_parsed_dt_df, start_parsetime, code_of_interest, daily=True):
    """
    ...
    for i in range(len(box_arr)):  # for all the boxes, (outermost index is box number)
        ...
        # Metric Code: will become the name of index (for dataframe)
        if daily:
            metric_code = m_date + " event_code: " + code_of_interest[0][-5:]  # last 5 digits of event_code

        else:
            metric_code = m_hour + " event_code: " + code_of_interest[0][-5:]  # last 5 digits of event_code
