...
# -*- coding: utf-8 -*-
"""
Based on Code By JHL & KN (Created on Thu Jun 25 15:07:10 2020)
Author: SC (Finalized on 05/31/2021)
    - updated code_dictionary to match delay discounting Arduino event codes
    - updated 0_RunMe.py to match new code dictionary + update valid_code_strings
    - updated paradigm_metrics to match new code dictionary
    - updated Step8b_latency + Tkinter_Selection_Ka_Modified files to adjust latency analysis to new event codes
    - all other files were taken from JHL + KN's original DIY-NAMIC analysis code

@author: Ichimaru
"""


#from Step_0_Settings import *
# import everything
...
if not os.path.isdir(OUTDIR):  #  only create output folder if output link does not already exist
    ...
    if not os.path.isdir(IOUTDIR_Rerranged_Data):  # only create output folder if output link  does not exist
...
varname.extend(['7070', '8070','9070','5521']) # initial timestamps
...
# total_df = filter_by_location(combined_metric_df, loc=["Left","Middle","Right"])   ## Change Here for Location!
# total_df = filter_by_location(combined_metric_df, loc=["Left","Middle","Right","Total"])   ## Change Here for Location!
# total_df = filter_by_location(combined_metric_df, loc=["Left","Right"])   ## Change Here for Location!


### Valid Code Strings (in a list)
valid_code_string = ["pokes_delay_window", "FC_pokes_delay_window", "pokes_iti_window","FC_pokes_iti_window", \
                     "pokes_response_window", "FC_pokes_response_window", "pokes_reward_window","FC_pokes_reward_window", \
                     "trials_omission", "FC_trials_omission", "FC_trials_incorrect", "trials_reward", "FC_trials_reward", \
                     "trials_valid_ports", "FC_trials_valid_ports", "pokes_paradigm_total", "pokes_initiation_window", \
                     "all_trials_total"]
